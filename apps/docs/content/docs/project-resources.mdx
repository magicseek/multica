...
## Optional connector-backed resources

Private connector profiles can register additional project resource types. For example, the RingCentral connector profile adds:
...
These resources are scope pointers only. They do not contain user tokens. Agents use task-scoped `multica connector ...` commands, and the server authorizes each call against the task, delegated user, provider policy, and attached project resource. See [RingCentral connectors](/ringcentral-connectors) for setup and command details.
