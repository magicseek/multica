...
Private or deployment-specific connectors are hidden unless an operator enables their profile explicitly. RingCentral connectors require all of these values:
...
| `RINGCENTRAL_GITLAB_API_BASE_URL` | empty | GitLab API base URL, for example `https://git.example.com/api/v4` |
| `RINGCENTRAL_GITLAB_WEB_BASE_URL` | empty | GitLab web base URL used for repository and merge request links |
| `RINGCENTRAL_JIRA_BASE_URL` | empty | Jira base URL |
| `RINGCENTRAL_WIKI_BASE_URL` | empty | Wiki base URL |
