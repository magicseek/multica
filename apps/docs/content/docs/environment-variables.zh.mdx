...
私有或部署专属的连接器默认隐藏，必须由运维显式启用 profile。RingCentral 连接器需要这些变量：
...
| `RINGCENTRAL_GITLAB_API_BASE_URL` | 空 | GitLab API base URL，例如 `https://git.example.com/api/v4` |
| `RINGCENTRAL_GITLAB_WEB_BASE_URL` | 空 | GitLab web base URL，用于仓库和 MR 链接 |
| `RINGCENTRAL_JIRA_BASE_URL` | 空 | Jira base URL |
| `RINGCENTRAL_WIKI_BASE_URL` | 空 | Wiki base URL |
