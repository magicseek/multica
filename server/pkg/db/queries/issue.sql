...
SELECT id, workspace_id, title, description, status, priority,
       ...
       workflow_override_definition_id
FROM issue
...
INSERT INTO issue (
    ...
    sqlc.narg('workflow_override_definition_id')
) RETURNING *;
...
UPDATE issue SET
    ...
    workflow_override_definition_id = sqlc.narg('workflow_override_definition_id'),
    updated_at = now()
WHERE id = $1
RETURNING *;
...
WHERE workspace_id = @workspace_id
  ...
  AND lower(btrim(regexp_replace(title, '[[:space:]]+', ' ', 'g'))) = @normalized_title
