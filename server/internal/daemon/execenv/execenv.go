...
// RepositoryBindingContextForEnv is the daemon-local binding summary used to
// materialize current-runtime local repositories into the task workdir.
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	logger.Info("execenv: prepared env", "root", envRoot, "remote_repos_available", len(task.Repos), "repositories_available", len(task.Repositories))
	return env, nil
}
