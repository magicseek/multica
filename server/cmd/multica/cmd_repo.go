...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := runtimeEnvValue("MULTICA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("MULTICA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := runtimeEnvValue("MULTICA_WORKSPACE_ID")
	agentName := runtimeEnvValue("MULTICA_AGENT_NAME")
	taskID := runtimeEnvValue("MULTICA_TASK_ID")
