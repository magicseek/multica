...
- User-created local bindings with `runtime_id` must target a runtime owned by the current user, and `daemon_id` must match that runtime's daemon id.
- Workspace-wide realtime events must not include `local_path` or private binding metadata.
- Agents publish task output metadata by writing an explicit `outputs.json` manifest. Non-chat tasks use `.multica/outputs.json` under their workdir; chat tasks use `$MULTICA_STRUCTURED_OUTPUT_DIR/outputs.json`, where the daemon sets `MULTICA_STRUCTURED_OUTPUT_DIR` to `.multica/chats/<chat_session_id>` under the selected workdir. The daemon uploads metadata only from the current task's explicit manifest path; it must not scan worktrees, fall back from chat-scoped manifests to shared legacy paths, or upload file contents.
...
- Binding create with `runtime_id` for another user's runtime -> `403`.
- Binding create with `runtime_id` and mismatched `daemon_id` -> `400`.
- Local directory repository create with inline binding failure -> no repository row is committed.
...
- Good: creating a local directory repository from Workspace Settings stores a `local_dir` repository and owner-visible binding atomically; other workspace members still do not see `local_path`.
...
- Bad: daemon scans the whole workdir and uploads discovered file lists without an explicit current-task `outputs.json` manifest.
...
- Task output metadata: daemon reads only `.multica/outputs.json` for non-chat tasks and `$MULTICA_STRUCTURED_OUTPUT_DIR/outputs.json` for chat tasks, handler replaces prior task output metadata on upload, user API lists metadata after workspace membership checks, and unsafe manifests are rejected.
