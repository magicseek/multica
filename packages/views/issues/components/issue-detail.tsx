...
import { workflowKeys } from "@multica/core/workflows";
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {propertiesOpen && <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 pl-2">
          ...
          <PropRow label={t(($) => $.detail.prop_workflow)}>
            <WorkflowPicker
              workflowId={issue.workflow_override_definition_id ?? null}
              onUpdate={handleUpdateField}
            />
          </PropRow>
          ...
          <div>
            ...
            <WorkflowRunViewer
              ...
              answerComposer={(request) => (
                <CommentInput
                  key={`workflow-answer-${request.id}`}
                  issueId={id}
                  draftKey={`workflow:${request.id}`}
                  onSubmit={async (content) => {
                    await api.answerWorkflowInputRequest(request.id, {
                      answer_text: content,
                      continue: true,
                    });
                    await queryClient.invalidateQueries({
                      queryKey: workflowKeys.all(wsId),
                    });
                  }}
                />
              )}
              commentComposer={
                <CommentInput
                  key={`workflow-comment-${id}`}
                  issueId={id}
                  draftKey={`workflow:${id}`}
